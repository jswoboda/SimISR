...
#        pdb.set_trace()
    ...
    def xyvecs(self,beamlist=None):
        ...
        return (xx,yy)
    def plotbeams(self,beamlist,plotall=True,filename=None):
        ...
        if plotall:
            ...
            (xx,yy) = angles2xy(azall,elall)
            plt.plot(xx,yy,'ko')
            plt.plot(xx,yy,'b.')
            plt.hold(True)
        
        (azvec,elvec) = self.azelvecs(beamlist)
        (xx2,yy2) = angles2xy(azvec,elvec)
        plt.plot(xx2,yy2,'ko')
        plt.plot(xx2,yy2,'y.')
        if filename != None:
...
def make_polax():    
    ...
    for iaz in azlines:
        ...
        (xx,yy) = angles2xy(azvec,elvec)
        plt.plot(xx,yy,'k--')
        if firstplot:
            plt.hold(True)
            firstplot=False
        (xt,yt) = angles2xy(azvec[-1],elvec[-1]-5)
