...
def sphereical2Cartisian(spherecoords):
    ...
    Parameters
    ----------
    spherecoords : ndarray
        A 3xN numpy array with rows of range (in km) azimuth (in degrees) and elevation (in degrees).

    Returns
    -------
    cartcoords : ndarray
        A 3xN numpy array with X, Y and Z in a cartisian coordinate space. The coordinates are in units of kilometers.
    """
    d2r = np.pi / 180
...
def wgs2ecef(WGS_COORDS):
    ...
    ----------
    WGS_COORDS : ndarray
        A 3xN numpy array (can also be a Nx3 array) with the latitude longitude and height coordinates. The matrix is broken up in the following way, the first row is latitude in degrees, the second row is longitude in degrees and the last row is height from the surface of the earth in meters.

    Returns
    -------
    ECEF_COORDS : ndarray
        A 3xN numpy array with X, Y and Z in ECEF coordinate space.  The coordinates are in units of meters.

    Reference
    ---------
    Vermeille H (2002) Direct transformation from geocentric coordinates to geodetic coordinates, Journal of Geodesy, vol. 76, no. 8, pp 451 - 454, Nov 2002, http://link.springer.com/article/10.1007%2Fs00190-002-0273-6
    """
    # %% Check Input
    d2r = np.pi / 180.0
...
def ecef2wgs(ECEF_COORDS):
    ...
    in degrees and meters. This has been compared to a working matlab version that has been comapred to the internal MATLAB function ecef2geodetic and has
    comparible results with a normalized difference on the order of 10^-23.

    Parameters
    ----------
    ECEF_COORDS : ndarray
        A 3xN numpy array (can also be a Nx3 array) with X, Y and Z in the ECEF coordinate space.  Coordinates are in units of meters.

    Returns
    -------
    WGS_COORDS : ndarray
        A 3xN numpy array  with the latitude longitude and height coordinates.  The matrix is broken up in the following way, the first row is latitude in degrees, the second row is longitude in degrees and the last row is height from the surface of the earth in meters.
...
def ecef2enul(ECEF, LatLongHeight):
    ...
    ----------
    ECEF : ndarray
        A 3xN matrix (can also be a Nx3 matrix) with X, Y and Z in ECEF coordinate space.  The  coordinates must be in meters.  The X,Y and Z coordinates are broken up where they are rows 1, 2, and 3 of the matrix respectively(assuming a 3xN array, if transposed this will be the columns.
    LatLonHeight : ndarray
        A 3xN matrix (can also be a Nx3 matrix) with the latitude longitude and height. These will be the coordinates that the ENU coordinate system is in reference to.  The matrix is broken up in the following way, the first row is latitude in degrees, the second row is longitude in degrees and the last row is the height in meters.

    Returns
    -------
    ENU : ndarray
        A 3xN matrix with the rotated vectors in meters refereced to east up from the original stated in the WGS coordinate given.  The east, north and up components are rows 1, 2 and 3 respectivly.
...
def ecef2enu4vec(ECEF, LatLong):
    ...
    them to an ENU coordinate system. This has been compared to a working matlab version that has been comapred to the internal MATLAB function and has comparible results with a normalized difference on the order of 10^-31.

    Parameters
    ----------
    ECEF : ndarray
        A 3xN numpy array (can also be a Nx3 array) with X, Y and Z in ECEF coordinate space. The coordinates are in meters.  The X,Y and Z coordinates are broken up where they are rows 1, 2, and 3 of the array respectively(assuming a 3xN array, if transposed this will be the columns.
    LatLon : ndarray
        A 2xN array (can also be a Nx2 matrix) with the latitude longitude. The matrix is broken up in the following way, the first row is latitude in degrees, the second row is longitude in degrees.

    Returns
    -------
    ENU : ndarray
        A 3xN numpy array with the rotated vectors in whatever unit the origial vectors were in. The east, north and up components are rows 1, 2 and 3 respecitivly.
...
def enu2ecefl(ENU, LatLongHeight):
    ...
    ----------
    ENU : ndarray
        A 3xN matrix (can also be a Nx3 matrix)with the rotated vectors in meters from the origin point stated in LatLongHeight. The east, north and up components are rows 1, 2 and 3 respecitivly.
    LatLonHeight : ndarray
        A 3xN matrix (can also be a Nx3 matrix) with the latitude longitude and height. These will be the coordinates that the ENU coordinate system is in reference to.  The matrix is broken up in the following way, the first row is latitude in degrees, the second row is longitude in degrees and the last row is the height in meters.
    ...
    -------
        ECEF : ndarray
        A 3xN matrix with X, Y and Z in ECEF coordinate space. The coordinates must be in meters.  The X,Y and Z coordinates are broken up        where they are rows 1, 2, and 3 of the matrix respectively.
...
def enu2ecef4vec(ENU, LatLong):
    ...
    Description
    This function will take a set of vectors (in terms of the mathematical structure) in ENU coordinates and rotate them to an ECEF coordinate system. This has been compared to a working matlab version that has been comapred to the internal MATLAB function and has comparible results with a normalized difference on the order of 10^-33.

    Parameters
    ----------
    ENU : ndarray
        A 3xN matrix (can also be a Nx3 matrix)with the rotated vectors in whatever unit the origial vectors were in.  The east, north and up components are rows 1, 2 and 3 respecitivly.
    LatLon : ndarray
        A 2xN matrix (can also be a Nx2 matrix) with the latitude longitude. The matrix is broken up in the following way, the first row is latitude in degrees, the second row is longitude in degrees.

    Returns
    -------
    ECEF : ndarray
    A 3xN matrix  with X, Y and Z in ECEF coordinate space.  The coordinates can be in what ever units the user wants.  The X,Y and Z coordinates are broken up where they are rows 1, 2, and 3 of the matrix respectively.
...
def enu2cartisian(ENUCOORDS):
    """This function will transform enu coordinates to Cartisian coordinates, the only difference being that ENU coordinates are in meters and Cartisian are in km.

    Parameters
    ----------
    ENUCOORDS : ndarray
        A 3xN numpy array with X, Y and Z in a ENU coordinate space. The coordinates are in units of meters.
...
def cartisian2enu(CARTCOORDS):
    """This function will transform Cartisian coordinates to ENU coordinates, the only difference being that ENU coordinates are in meters and Cartisian are in km.
...
def nthroot(X, N):
    """This will determine the nth root of a number even if its negative. This needed for the ecef to wgs conversion.

    Parameters
    ----------
    X : ndarray
        Array of numbers that will have the nth root taken.
    N : float
        Root to be taken.
    ...
    -------
    xroot : ndarray
        Nthroot of X but the negative numbers will have the negative nth root.
    """
    xlog = X < 0
...
def angles2xyz(az, el):
    """Creates x and y coordinates from az and el arrays. Elevation angle measured from the z=0 plane. Azimuth angle from x=0 and goes clockwise.
