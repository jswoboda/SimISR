...
"""
radarsystools
This set of functions can be used to calculate expect SNR and variance of ISR 
data.
...
class RadarSys(object):
    ...
    def __init__(self,sysdict,rng = np.arange(100.0,600.0,100.0),Kpulse = 1):
    ...
    def printvar(self,ne = np.array([1e11]),te=np.array([1e3]),ti=np.array([1e3])):
        ...
        #%% Set up and Make print out
        print "Variances for the follwing Ne with Number of pulses = {0:d}".format(Kpulse)
        # make strings for each Ne
        nestrs = ['{:.2g}m^-3 '.format(i) for i in ne]
        nestrs.insert(0,'Range ')
        lstoflst = [nestrs]
        ...
        for irng,vrng in enumerate(rng):
            ...
            varliststr = ['{:.2e}W^2 '.format(i) for i in vardata[irng]]
            varliststr.insert(0,rngstr)
            lstoflst.append(varliststr)
        # Determine    
        cols = zip(*lstoflst)
        col_widths = [ max(len(value) for value in col) for col in cols ]
        format = ' '.join(['%%-%ds' % width for width in col_widths ])
...
#    
#    return np.sqrt(Kpulse)*powdata/noisedata
#    
#    
...
#    powdata = powcalc(sysdict,rng,ne,te,ti)
#    noisedata = noisepow(sysdict['Tsys'],sysdict['BandWidth'])
#    return (powdata+noisedata)**2/Kpulse
...
if __name__== '__main__':
    ...
    sensdict['tau'] = 320e-6
    r_sys = RadarSys(sensdict,Kpulse = 100)
    r_sys.printsnr(ne = np.array([1e11,2e11]),te=np.array([1e3,2e11]),ti=np.array([1e3,2e11]))
    r_sys.printvar(ne = np.array([1e11,2e11]),te=np.array([1e3,2e11]),ti=np.array([1e3,2e11]))
